...
renderCUDA_count(
    ...
    const float* __restrict__ bg_color,
    float* __restrict__ out_color,
    int max_contributions_per_pixel, // 추가된 파라미터
    int* __restrict__ per_pixel_gaussian_ids, // 추가된 버퍼
    float* __restrict__ per_pixel_gaussian_contributions // 추가된 버퍼
)
{
    // 현재 타일과 픽셀 범위 식별
    auto block = cg::this_thread_block();
    ...
    // 각 픽셀별 기여한 가우시안의 수를 추적하기 위한 변수
    int contribution_count = 0;
    int per_pixel_offset = pix_id * max_contributions_per_pixel;
    ...
    for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
    {
        ...
        // 현재 배치(iterate over current batch)
        for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
        {
            contributor++;
            ...
            // 기여도 저장 (최대 제한 내에서)
            if (contribution_count < max_contributions_per_pixel)
            {
                per_pixel_gaussian_ids[per_pixel_offset + contribution_count] = collected_id[j];
                per_pixel_gaussian_contributions[per_pixel_offset + contribution_count] = alpha * T;
                contribution_count++;
            }
    ...
    if (inside)
    {
        ...
        for (int ch = 0; ch < CHANNELS; ch++)
            out_color[ch * H * W + pix_id] = C[ch] + T * bg_color[ch];
        // 필요에 따라 contribution_count를 저장할 수 있습니다.
        // per_pixel_contribution_counts[pix_id] = contribution_count;
    }
}
...
void FORWARD::count_gaussian(
	...
	float* important_score,
	float* out_color,
	const int max_contributions_per_pixel,
	int* per_pixel_gaussian_ids,  // 추가된 버퍼
	float* per_pixel_gaussian_contributions  // 추가된 버퍼
)
{
	renderCUDA_count<NUM_CHANNELS> << <grid, block >> > (
		ranges,
		...
		bg_color,
		out_color,
		max_contributions_per_pixel,  // 새로운 파라미터
		per_pixel_gaussian_ids,  // 새로운 버퍼
		per_pixel_gaussian_contributions  // 새로운 버퍼
	);
}
