	...
	const bool f_count)
{
  ...
  // Initialize gaussians_count and important_score 
  ...
  // Additional tensors for storing per-pixel Gaussian contributions
  const int max_contributions_per_pixel = 100; // Maximum number of contributions to track per pixel
  torch::Tensor per_pixel_gaussian_ids = torch::full({H, W, max_contributions_per_pixel}, -1, int_opts);
  torch::Tensor per_pixel_gaussian_contributions = torch::full({H, W, max_contributions_per_pixel}, 0.0, float_opts);
  ...
  if(P != 0)
  {
	  ...
	  // Overloaded forward function, added gaussians_count, important_score, and per-pixel contributions
	  rendered = CudaRasterizer::Rasterizer::forwardCount(
		...
		per_pixel_gaussian_ids.contiguous().data<int>(),
		per_pixel_gaussian_contributions.contiguous().data<float>(),
		radii.contiguous().data<int>(),
		debug
	  );
  }

  // Return per-pixel Gaussian contributions along with other data
  return std::make_tuple(gaussians_count, important_score, rendered, out_color, radii, geomBuffer, binningBuffer, imgBuffer, per_pixel_gaussian_ids, per_pixel_gaussian_contributions);
}
